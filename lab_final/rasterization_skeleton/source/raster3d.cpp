...
int vertexCount = 9468;
std::vector<glm::vec3> vertexNormals(vertexCount, glm::vec3(0));
...
int main(int argc, char** argv)
{
    ...
    for (uint32_t i = 0; i < ntris; ++i) {
        ...
        vertexNormals[nvertices[i * 3]] += faceNormal;
        vertexNormals[nvertices[i * 3 + 1]] += faceNormal;
        vertexNormals[nvertices[i * 3 + 2]] += faceNormal;
    }

    // 모든 vertex normal normalize (한 번만)
        ...
        for (uint32_t y = y0; y <= y1; ++y) {
            for (uint32_t x = x0; x <= x1; ++x) {
                ...
                if (w0 == true && w1 == true && w2 == true) { //inside
                    ...
                    float a2 = area(v0Raster, v1Raster, pixelSample); //v0Raster, v1Rasterd에서 빠진 것이 2이니까 a2
                    ...
                    float z; //pixelSample의 z값을 파악해야함 : pixelSample의 카메라까지와의 거리

                    float oneOverZ = v0Raster.z * a0 + v1Raster.z * a1 + v2Raster.z * a2; 
                    z = 1.0f / oneOverZ; //이 값이 픽셀의 depth

                    //code here


                    //depth버퍼에는 해당 픽셀의 가장 가까운 거리값(카메라까지의)이 들어있음. 아주 중요함
                    if (z < depthBuffer[y * imageWidth + x]) {
                        // vertexCount = cow 모델의 전체 vertex 개수
                        
                        

                        glm::vec3 n0 = vertexNormals[nvertices[i * 3]];
                        glm::vec3 n1 = vertexNormals[nvertices[i * 3 + 1]];
                        glm::vec3 n2 = vertexNormals[nvertices[i * 3 + 2]];

                        glm::vec3 pixelNormal = glm::normalize(a0 * n0 + a1 * n1 + a2 * n2);
                        ...
                        glm::vec3 n = (pixelNormal + glm::vec3(1.0f)) / glm::vec3(2);
                        //n -> [-1,1] , x , y , z 모두 [-1,1] -> [0,1]
